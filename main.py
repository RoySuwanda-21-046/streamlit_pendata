...
with tab1:
    ...
    st.markdown("""
    Link Dataset:
    <a href="https://archive.ics.uci.edu/ml/datasets/Breast+Cancer+Coimbra> </a>
    """, unsafe_allow_html=True)
...
with tab4:
    st.write("""
    <h5>Model Terbaik yaitu MLP dengan tingkat akurasi sebesar </h5>
    <br>
    """, unsafe_allow_html=True)
    st.write("""
    ...
    with col1:    
        HOMA = st.text_input('Input nilai HOMA')
    with col2:
        Leptin = st.text_input('Input nilai Leptin')
    with col2:
        Adiponectin = st.text_input('Input nilai Adiponectin')
    with col2:
        Resistin = st.text_input('Input nilai Resistin')
    with col2:
        MCP1 = st.text_input('Input nilai MCP-1')
